...
{
    public record CreateCommentRequest(string Text, string UserId, Guid PostId) : IRequest<CreateCommentResponse>;
    ...
    public class CreateCommentHandler(ICommentService service) : IRequestHandler<CreateCommentRequest, CreateCommentResponse>
    {
        ...
        {
            return new(await service.Create(request.Text, request.UserId, request.PostId));
        }
    }
}
